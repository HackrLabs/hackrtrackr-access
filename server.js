...
sp.on("open", function(){
  ...
  if(readData.indexOf("*") != -1) {
    var arduinoQueryString = readData.split(',')[0]
    if(arduinoQueryString === "@^0." || arduinoQueryString === "@^0:"){
      switch(arduinoQueryString) {
        case '@^0.':
          console.log('Door Opened by Button')
    break;
        case '@^0:':
                  console.log('Door Closed after button push.')
                    break;
      }
    } else {
      ...
      var cardCheckQuery = "SELECT m.memberid, m.firstname, m.lastname, isactive, c.cardid from members m left join cards c on m.memberid = c.memberid where c.cardid = '" + card + "'";
      pgClient.query(cardCheckQuery, function(err, result) {
        if(err) {
